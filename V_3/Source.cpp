...
vector<int> blobanalysis(Blob blobanal){
	...
	int j = 0;
	for (j < blobanal.pixels.size(); j++;) {
			if ((blobanal.pixels.at(j).at(0) < lowestx)) {
				lowestx = blobanal.pixels.at(j).at(0);
			}
			if ((blobanal.pixels.at(j).at(1) < lowesty)) {
				lowesty = blobanal.pixels.at(j).at(1);
			}
			if ((blobanal.pixels.at(j).at(0) > biggestx)) {
				biggestx = blobanal.pixels.at(j).at(0);
			}
			if ((blobanal.pixels.at(j).at(1) > biggesty)) {
				biggesty = blobanal.pixels.at(j).at(1);
			}
			averagex += blobanal.pixels.at(j).at(0);
			averagey += blobanal.pixels.at(j).at(1);
		}
	averagex = averagex / j;
	averagey = averagey / j;

	vector<int> coordinates;
	coordinates.push_back(lowestx);
	coordinates.push_back(biggestx);
	coordinates.push_back(lowesty);
	coordinates.push_back(biggesty);
...
vector<Boat> boatFinder(vector<Blob> blobs, Mat image) {
	vector<Boat> boats;
	for (int i = 0; i < blobs.size(); i++) {
		if (blobs.at(i).pixels.size() > 10) {
			vector<int> blobcoords;
			blobcoords = blobanalysis(blobs.at(i));
			int width = blobcoords.at(1) - blobcoords.at(0);
			int height = blobcoords.at(3) - blobcoords.at(2);
			double ratio;
			bool direction;
			if (width < height) {
				ratio = height / width;
			...
			boatRatios.push_back(vector<double>{ 5, 7.5 });
			boatRatios.push_back(vector<double>{ 4, 5.8 });
			boatRatios.push_back(vector<double>{ 3, 4.0 });
			boatRatios.push_back(vector<double>{ 2, 2.3 });
			
			double bestFit = 10;
			int size;

			for (int j = 0; j < boatRatios.size(); j++) {
				if (abs(boatRatios.at(j).at(1) - ratio) < bestFit) {
					bestFit = abs(boatRatios.at(j).at(1) - ratio);
					size = boatRatios.at(j).at(0);
				}
			}
		}
	}
